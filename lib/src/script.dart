...
import 'package:wallet_hd/src/check_types.dart';
import 'package:wallet_hd/src/ecurve.dart';
import 'package:wallet_hd/src/op.dart';

import 'package:wallet_hd/src/push_data.dart' as pushData;
...
Uint8List compile(List<dynamic> chunks) {
  int bufferSize = chunks.fold(0, (acc, chunk) {
    if (chunk is int) return acc + 1;
    if (chunk.length == 1) {
      return acc;
    }

    final res = acc + pushData.encodingLength(chunk.length) + chunk.length;
    return res as int;
  });
...
Uint8List bip66encode(r, s) {
  ...
  final res = 6 + lenR + lenS;
  var signature = new Uint8List(res as int);
